...
This file is about prompt structure, specificity, and iteration for the `generate_image` tool.
...
- For small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes, set `quality: "high"`.
...
## Transparent images
- Try `background: "transparent"` first. Some sizes/subjects ignore it.
- If transparent does not honor for your case, fall back to chroma-key prompting:
  ...
  - Remove the key in post with the bundled `scripts/remove_chroma_key.py`:
    ```bash
    python <skill-dir>/scripts/remove_chroma_key.py \
      --input <generated.png> --out <final.png> \
      --auto-key border --soft-matte \
      --transparent-threshold 12 --opaque-threshold 220 --despill
    ```
    Requires Python 3 with `Pillow`. Use `--soft-matte` and `--despill` for antialiased edges; hard tolerance-only removal is mainly for flat pixel-art or exact-color fixtures.
- Complex subjects with hair, fur, glass, smoke, liquids, translucent materials, reflective objects, or soft shadows tend to be brittle for chroma-key removal — set expectations with the user before iterating.

## Quality, size, and aspect
- `quality: "low"` for fast drafts and thumbnails; `medium` or `high` for final assets and dense text.
- `size: "auto"` lets the model pick. For specific aspect ratios use `1024x1024`, `1024x1536` (portrait), or `1536x1024` (landscape). Larger sizes consume more credit and are slower.
...
- infographic-diagram: Define the audience and layout flow; label parts explicitly; require verbatim text; set `quality: "high"` for dense labels.
...
- background-extraction: For simple opaque subjects, try `background: "transparent"`. If that does not honor, request a clean cutout on a perfectly flat chroma-key background; crisp silhouette; generous padding; no shadows; no halos; preserve label text exactly; no restyling, then run `scripts/remove_chroma_key.py`. Complex subjects (hair, glass, smoke) may not extract cleanly — flag this to the user before iterating.

...
Ready-to-adapt prompt recipes for the `generate_image` tool.
...
The labeled lines (`Asset type`, `Input images`, etc.) are prompt scaffolding for the model, not parameters of the tool itself. Pass the entire scaffolded text as the `prompt` argument.
...
Post-process note: try `background: "transparent"` first. If the model does not honor it for your size/subject, generate against a flat chroma-key as above and remove the key with the bundled `scripts/remove_chroma_key.py --auto-key border --soft-matte --despill`. Complex subjects with hair, fur, glass, smoke, liquids, translucent materials, reflections, or soft shadows are brittle for chroma-key removal — set expectations with the user first.
